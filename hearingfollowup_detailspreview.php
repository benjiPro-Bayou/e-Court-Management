...
require_once("include/hearingfollowup_variables.php");
...
$page_layouts["hearingfollowup_detailspreview"] = $layout;
...
if($mastertable == "administrative hearing")
{
	$where = "";
		$formattedValue = make_db_value("HID",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$where .= $pageObject->getFieldSQLDecrypt("HID") . " is null";
	else
		$where .= $pageObject->getFieldSQLDecrypt("HID") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	HID - 
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("HID")))
				$class = ' rnr-field-number';
			$row["HID_class"] = $class;
	//	requiredDocOrWitness - Document Download
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("requiredDocOrWitness", $data, $keylink);
			$row["requiredDocOrWitness_value"] = $value;
			$format = $pSet->getViewFormat("requiredDocOrWitness");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("requiredDocOrWitness")))
				$class = ' rnr-field-number';
			$row["requiredDocOrWitness_class"] = $class;
...
$xt->load_template(GetTemplateName("hearingfollowup", "detailspreview"));
